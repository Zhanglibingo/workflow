...
void process(WFHttpTask *server_task)
{
	...
	std::string mystr = "<h1>Negative news</h1>";
	resp->append_output_body(mystr.c_str(), mystr.length());

	FILE *file1 = fopen("score.txt", "r");
	FILE *file2 = fopen("href_title.txt", "r");
	char buf1[8192];
	char buf2[8192];
	char buf3[8192];
	char buf4[8192];
	while (fgets(buf1, sizeof(buf1), file1))
	{
		fgets(buf2, sizeof(buf2), file2);
		fgets(buf3, sizeof(buf3), file2);
		if(atof(buf1) >= 0.0) continue;
		len = snprintf(buf4, 8192, "<li><a href=\"%s\" target=\"_blank\">%s, score:%s </a></li>", buf2, buf3, buf1);
		resp->append_output_body(buf4, len);
	}

	// mystr = "<a href=\"https://www.runoob.com/\" target=\"_blank\">runoob.com </a>";
